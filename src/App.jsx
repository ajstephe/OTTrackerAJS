                          ...
                          {week.map((date,di)=>{
                            ...
                            return (
                              <button key={info.ds} onClick={()=>{
                                ...
                                <span style={{fontSize:isWide?'16px':'13px',fontWeight:info.hasOT?900:600,color:info.isRecordOnly?'#475569':info.hasOT?(info.isFullySubmitted?'#15803d':'#b91c1c'):'#94a3b8',lineHeight:1}}>{date.getDate()}</span>
                                {info.totalHrs>0&&(
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginTop:'12px',paddingTop:'12px',borderTop:'1px solid #f1f5f9'}}>
                      <div style={{display:'flex',flexDirection:'column',alignItems:'flex-start',gap:'6px'}}>
                        ...
                        <div style={{display:'flex',alignItems:'center',gap:'5px'}}>
                          ...
                          <span style={{fontSize:'13px',fontWeight:700,color:'#64748b'}}>OT/PA Counted Other Period</span>
                        </div>
                      </div>
...
// ─── deprecation notice ─────────────────────────────────────────────────────
// This app has been superseded — everyone should move to the new one. Shown
// as a fixed overlay on top of literally everything else the app could
// render (loading spinner, auth screens, the main app itself), since it
// sits in Root below rather than inside App's own conditional branches.
// Deliberately has no "don't show again" / localStorage dismissal — the
// whole point is that it reappears every time the app is opened, not just
// once. Closing it (via the X) only dismisses it for the current visit.
function DeprecationNotice() {
  const [open, setOpen] = useState(true);
  if (!open) return null;
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.6)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px',fontFamily:"'DM Sans',system-ui,sans-serif"}}>
      <div style={{background:'#fff',borderRadius:'18px',padding:'28px 26px 24px',maxWidth:'420px',width:'100%',boxShadow:'0 20px 60px rgba(0,0,0,0.35)',position:'relative',boxSizing:'border-box'}}>
        <button onClick={()=>setOpen(false)} aria-label="Close" style={{position:'absolute',top:'14px',right:'14px',background:'#f1f5f9',border:'none',borderRadius:'9px',width:'30px',height:'30px',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>
          <Ico n="x" s={16} c="#64748b"/>
        </button>
        <div style={{background:'#fffbeb',border:'1px solid #fde68a',borderRadius:'13px',padding:'16px 16px',marginBottom:'4px'}}>
          <div style={{fontWeight:900,fontSize:'15px',color:'#92400e',marginBottom:'8px'}}>This app is no longer supported</div>
          <div style={{fontSize:'13px',color:'#92400e',lineHeight:1.6,marginBottom:'12px'}}>Please use the new version instead — this one will not receive further updates.</div>
          <a href="https://rasptrackas.vercel.app" target="_blank" rel="noopener noreferrer" style={{display:'block',textAlign:'center',background:'#2563eb',color:'#fff',fontWeight:800,fontSize:'13px',padding:'11px',borderRadius:'11px',textDecoration:'none'}}>
            rasptrackas.vercel.app
          </a>
        </div>
      </div>
    </div>
